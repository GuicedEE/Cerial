# 🔄 Starter Prompt — Guiced Cerial Library Rules Update (Adopt Rules Repository in an Existing Project)

This document captures the settings from `PROMPT_ADOPT_EXISTING_PROJECT.md` and adapts them for the GuicedEE cerial section inside `rules/generative/backend/guicedee/cerial`. Use it when copying the Rules Repository into a host project or updating the Guiced Cerial library rules to remain forward-only and modular.
...
- Repository URL / local path: https://github.com/GedMarc/GuicedCerial.git
- Org and project name: GuicedEE / Guiced Cerial
- Short description: A Serial Port utility utilizing jSerialComm
